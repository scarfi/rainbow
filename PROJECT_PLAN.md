...
The first offline application slice is implemented locally: SvelteKit, Dexie storage, equipment setups, session drafts, notes, standard-face selection (40, 60, 80, and 122 cm), 10-zone scoring, four-point Beursault scoring with honneurs/points/chapelets/noirs, unfinished-end persistence, a start/stop timer, a full-viewport shooting interface, Beursault and progression presets, a training journal with a yearly arrow activity calendar and day filtering, JSON export, and an application-shell service worker. English and French interfaces are included. Automated persistence and cache tests pass; actual device offline verification remains open. Contextual CSV/JSON exports and an optional Supabase email/password authentication interface are implemented locally. The local Supabase configuration is connected; email/password authentication and signup are enabled with email confirmation required. Email delivery, redirects, and end-to-end login remain to be verified. Account-isolated offline synchronization is implemented locally with a durable queue, version-checked RPC, retry handling, and conflict copies. The owner confirmed the production migration succeeded on 2026-09-09. Real-device cross-account synchronization verification remains open. Media and community features are not implemented.

The repository is connected to GitHub. Implementation is integrated through feature branches and pull requests. GitHub Actions validates changes and deploys main to GitHub Pages when Pages is enabled. The project owner has created a Supabase project. Its MCP tools are not currently available in this task. The supplied publishable key is configured in ignored local environment settings and accepted by the public Auth settings endpoint. Email delivery, redirects, and live login verification remain pending.
